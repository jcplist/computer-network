...
fn get_handler(req: &mut Request, res: &mut Response) {
    if req.path == "/" {
        ...
        res.data(&buf);
    } else if req.path == "/70000.mp3" {
        let mut f = File::open("./static/70000_10min.mp3").unwrap();

        let len = f.metadata().unwrap().len();
        if let Some(range_raw) = req.attr.get("Range") {
            let r = Regex::new(r"bytes=(\d+)-(\d*)").unwrap();
            let caps = r.captures(range_raw).unwrap();
            let mut start = caps.get(1).unwrap().as_str().parse().unwrap();
            let mut end = if caps.get(2).unwrap().as_str() == "" {
                len
            } else {
